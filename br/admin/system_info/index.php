		...
		<div class="card-body">
			<form action="" id="system-frm">
				...
				<div class="form-group">
					...
					<small><i>Escolha fazer upload de novas imagens de banner</i></small>
				</div>
				<?php
...
<script>
	...
	function delete_img($path) {
		...
		$.ajax({
			...
			error: err => {
				console.log(err)
				alert_toast("Ocorreu um erro ao excluir uma imagem", "error");
				end_loader()
			},
			success: function(resp) {
				...
				if (typeof resp == 'object' && resp.status == 'success') {
					...
					alert_toast("Imagem excluída com sucesso", "success");
				} else {
					console.log(resp)
					alert_toast("Ocorreu um erro ao excluir uma imagem", "error");
				}
	...
	$(document).ready(function() {
		$('.rem_img').click(function() {
			_conf("Tem certeza de excluir esta imagem permanentemente?", 'delete_img', ["'" + $(this).attr('data-path') + "'"])
		})
